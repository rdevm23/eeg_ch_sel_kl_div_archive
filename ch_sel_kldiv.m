...
all_ranks = rankings(n_channels,all_klds,n_subs);
all_ranks(1).filter = filters_ON;
...
function all_ranks = rankings(n_channels,all_klds,n_subs)
all_ranks = struct;
for s = 1:n_subs
...
for i = 1: n_subs
    ...
    cz_sig = reshape(signals(11,:,:),[n_trials,n_samples]);
    c3_sig = reshape(signals(9,:,:),[n_trials,n_samples]);
    c4_sig = reshape(signals(13,:,:),[n_trials,n_samples]);
